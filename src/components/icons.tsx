/**
 * A drawable mark: path data plus the box it was drawn in.
 *
 * Simple Icons are all 24x24, but an inlined logo keeps its own proportions —
 * the AWS wordmark is 1.67:1 — so the viewBox travels with the path instead of
 * being assumed.
 */
export type Mark = {
  path: string;
  viewBox: string;
  /**
   * Brand colour, six hex digits without the `#`, used only for the hover
   * bloom in `TechBadge`. Omitted where one colour would misrepresent the mark
   * — Microsoft's four-square is four colours, and flattening it to any one of
   * them would be wrong rather than merely plain.
   */
  hex?: string;
};

/**
 * Microsoft's four-square mark, used for Microsoft Power Platform.
 *
 * Four equal 11.377 squares with a 1.246 gutter, which is the official
 * geometry. The real logo is four colours; badges render in `currentColor`, and
 * the silhouette is what carries the recognition at 12px.
 */
export const MICROSOFT_MARK: Mark = {
  viewBox: "0 0 24 24",
  // Deliberately no `hex` — see the note on `Mark`.
  path: "M0 0h11.377v11.377H0zM12.623 0H24v11.377H12.623zM0 12.623h11.377V24H0zM12.623 12.623H24V24H12.623z",
};

/**
 * The AWS wordmark, from the devicon set (MIT).
 *
 * The viewBox is the path's measured bounding box rather than devicon's
 * 0 0 128 128 canvas — the drawn mark occupies only the middle band of that, so
 * the full canvas would render it far smaller than its neighbours. Measured
 * with `getBBox()`, not eyeballed.
 */
export const AWS_MARK: Mark = {
  viewBox: "0.7 26.15 126.6 75.7",
  // The smile's orange, not the wordmark's squid ink. Both are AWS brand
  // colours; this is the one that survives being re-lit for a dark surface.
  hex: "FF9900",
...
/**
 * Brand marks that neither lucide-react v1 nor simple-icons ships.
 *
 * Simple Icons has dropped LinkedIn, Amazon and Microsoft over trademark
 * policy — a constraint on redistributing an icon *set*, not on a portfolio
 * naming the technologies it works with — so those paths are inlined here and
 * above.
 *
 * This is the standard 24×24 "in" glyph; LinkedIn's brand guidelines permit
 * using it to link to a profile. If you need to change it, take the official
 * SVG from LinkedIn's brand resources rather than editing these coordinates.
 */
