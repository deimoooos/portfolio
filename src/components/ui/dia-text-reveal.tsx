...
export function DiaTextReveal({
  ...
  // Memoised so the measuring effect below can depend on it honestly instead of
  // on a `text.join()` expression the linter cannot check statically.
  ...
  // Magic UI assigns this during render, which React 19 rejects
  // (`react-hooks/refs`): a render pass is not allowed to mutate a ref. Same
  // intent, moved to an effect that runs after every render — the animation
  // loop still reads the latest props without them becoming dependencies of
  // the loop itself. `useRef` above seeds it, so the first play already has
  // the right values.
  useEffect(() => {
    optsRef.current = {
      colors,
  ...
  // Seeded at the END of the sweep, not the start.
  //
  // `buildGradient` returns a flat two-stop of `textColor` once the band has
  // passed, and a fully transparent fill before it arrives — and this is
  // `background-clip: text` over `color: transparent`, so the start value
  // renders the <h1> INVISIBLE. That is what the server sends: measured, the
  // SSR markup carries `linear-gradient(90deg, ... transparent 0%, transparent
  // 100%)`, so the name does not exist until React hydrates and motion runs.
  // Seeding the end state makes the sweep a pure enhancement: the name paints
  // in `--foreground` from the first frame, and `play()` sets SWEEP_START
  // itself before animating.
  ...
  // Declared *inside* an effect, not during render. It writes `stopRef` and
  // `timerRef`, and a function that mutates refs is not something a render pass
  // may even define (`react-hooks/immutability`). It stays a ref rather than a
  // `useCallback` because `onComplete` calls it recursively to advance to the
  // next string.
  useEffect(() => {
    playRef.current = () => {
